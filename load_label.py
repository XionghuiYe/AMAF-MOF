def load_label(dataset_name, images, subjects, subjectsVideos, codeFinal):
    ...
    emotion_list = ['repression', 'anger', 'contempt', 'disgust', 'fear', 'sadness', 'negative', 'happiness',
                    'positive', 'surprise', 'others', 'other']
    ...
    for sub_video_each_index, sub_vid_each in enumerate(subjectsVideos):
        ...
        for videoIndex, videoCode in enumerate(sub_vid_each):
            ...
            for i, row in codeFinal.iterrows():
                if (str(row['subjectCode']) == str(
                        subjects[sub_video_each_index])):  # S15, S16... for CAS(ME)^2, 001, 002... for SAMMLV
                            ...
                            if (row['emotion'] in emotion_list):  # check emotion
                                ...
                                if (row['offset'] == 0):  # Take apex if offset is 0
                                    ...
                                    if (dataset_name == 'CASME2'):
                                        if (
                                        isinstance(row['apex'], int)):  # CASME2 has one sample has '/' at apex frame
                                            if ([int(row['onset'] - 1), int(row['apex']) - 1,
                                                 int(row['offset']) - 1] != [65, 110,
                                                                             110]):  # CASME2 has one sample has index error
                                                on_off.append([int(row['onset'] - 1), int(row['apex'] - 1),
                                                               int(row['offset']) - 1])
                                    elif (int(row[
                                                  'onset']) != 0):  # Ignore the samples that is extremely long in SAMMLV, suggested in other papers
                                        on_off.append(
                                            [int(row['onset'] - 1), int(row['apex'] - 1), int(row['offset'] - 1)])
