...
def spotting(result, total_gt_spot, subject_count, p, metric_final, spot_multiple, k_p, final_samples,
    ...
    for videoIndex, video in enumerate(final_samples[subject_count - 1]):
        ...
        score_plot = np.array(result[prev:prev + len(
            final_dataset_spotting[countVideo + videoIndex])])  # Get related frames to each video
        ...
        if spot_multiple:  # 是否检测多个峰值，单个的话提供给预测的excel用
            ...
            for peak in peaks:
                preds.append(
                    [peak - k_p, 0, peak + k_p, 0, 0, 0, peak])  # Extend left and right side of peak by k frames
        else:
        ...
        # 保存图形
        plt.savefig(f'video_{countVideo} + {videoIndex}.png')
        plt.close()  # 显式关闭图形窗口
...
def origi_spotting(result, total_gt_spot, subject_count, p, metric_final, spot_multiple, k_p, final_samples, final_dataset_spotting):
    prev=0
    pred_subject = []
    gt_subject = []
    metric_video = MeanAveragePrecision2d(num_classes=1)
    for videoIndex, video in enumerate(final_samples[subject_count-1]):
        ...
        if(spot_multiple):                                  #是否检测多个峰值，单个的话提供给预测的excell用
            peaks, _ = find_peaks(score_plot_agg, height=threshold, distance=k_p)
            if(len(peaks)==0): #Occurs when no peak is detected, simply give a value to pass the exception in mean_average_precision
                preds.append([0, 0, 0, 0, 0, 0, 0])
            for peak in peaks:
...
def my_torch_history_plot(history_recog, filename):
    f, ax = plt.subplots(1,2,figsize=(15,3))
    #Recog Loss vs Epochs
    ax[0].plot(history_recog['train_loss'])
    ax[0].plot(history_recog['val_loss'])
    ax[0].set_title('Recog Loss/Epochs')
    ax[0].set_ylabel('Recog Loss')
    ax[0].set_xlabel('Epoch')
    ax[0].legend(['train_loss','val_loss'], loc='upper left')
    #Recog Accuracy vs Epochs
    ax[1].plot(history_recog['train_acc'])
    ax[1].plot(history_recog['val_acc'])
    ax[1].set_title('Recog Accuracy/Epochs')
    ax[1].set_ylabel('Recog Accuracy')
    ax[1].set_xlabel('Epoch')
    ax[1].legend(['train_accuracy','val_accuracy'], loc='upper left')

    plt.savefig(f"{filename}.png")

def torch_history_plot(history_spot, history_recog, filename):
    f, ax = plt.subplots(1,4,figsize=(15,3))
    #Spot Loss vs Epochs
    ax[0].plot(history_spot['train_loss'])
    ax[0].plot(history_spot['val_loss'])
    ax[0].set_title('Spot Loss/Epochs')
    ax[0].set_ylabel('Spot Loss')
    ax[0].set_xlabel('Epoch')
    ax[0].legend(['train_loss','val_loss'], loc='upper left')
    #Spot MAE vs Epochs
    ax[1].plot(history_spot['train_mae'])
    ax[1].plot(history_spot['val_mae'])
    ax[1].set_title('Spot MAE/Epochs')
    ax[1].set_ylabel('Spot MAE')
    ax[1].set_xlabel('Epoch')
    ax[1].legend(['train_mae','val_mae'], loc='upper left')
    #Recog Loss vs Epochs
    ax[2].plot(history_recog['train_loss'])
    ax[2].plot(history_recog['val_loss'])
    ax[2].set_title('Recog Loss/Epochs')
    ax[2].set_ylabel('Recog Loss')
    ax[2].set_xlabel('Epoch')
    ax[2].legend(['train_loss','val_loss'], loc='upper left')
    #Recog Accuracy vs Epochs
    ax[3].plot(history_recog['train_acc'])
    ax[3].plot(history_recog['val_acc'])
    ax[3].set_title('Recog Accuracy/Epochs')
    ax[3].set_ylabel('Recog Accuracy')
    ax[3].set_xlabel('Epoch')
    ax[3].legend(['train_accuracy','val_accuracy'], loc='upper left')
...
#                 most_common_emotion, _ = Counter(pred_emotion_list).most_common(1)[0]
                cur_pred_single.append(pred_emotion_list.item())
                ...
                most_common_emotion, _ = Counter(pred_emotion_list).most_common(1)[0]
                cur_pred.append(most_common_emotion.item())
                ...
                most_common_emotion, _ = Counter(pred_emotion_list).most_common(1)[0]
                cur_pred_window.append(most_common_emotion.item())
                
                pred_gt_recog.append(argmax(pred_emotion[video_index][final_samples[subject_count-1][video_index][0][0]]).item()) #Predicted emotion on gt onset label
...
def plot_confusion_matrix_from_values(TP, FP, FN, TN):
    # 构建混淆矩阵
    cm = np.array([[TN, FP],
                   [FN, TP]])
...
#                     print('Total Samples:', num_samples, '| F1-score:', round(f1_recog, 4), '| Average Recall:', round(recall_recog, 4), '| Average Precision:', round(precision_recog, 4))
        ...
        plot_confusion_matrix_from_values(TP_all, FP_all, FN_all, TN_all)
...
def create_directory(train, dataset_name):
    path_main = 'F:\\my_N_Weights'
    path_dataset = 'F:\\my_N_Weights\\' + dataset_name
    path_spot = 'F:\\my_N_Weights\\' + dataset_name + '\\' + 'spot'
    path_recog = 'F:\\my_N_Weights\\' + dataset_name + '\\' + 'recog'
    path_spot_recog = 'F:\\my_N_Weights\\' + dataset_name + '\\' + 'spot_recog'
    if train:
