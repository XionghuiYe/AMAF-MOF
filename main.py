...
from my_V1_train_evaluate import pytorch_train_test, final_evaluation
from training_utils import create_directory
...
def main(config):
    ...
    recog_save_path = os.path.join(dataset_name + "_dataset_recognition.pkl")
    # pickle.dump(final_dataset_recognition, open(recog_save_path, "wb")) # To save time when needed to run for several attempts
    final_dataset_recognition = pickle.load(open(recog_save_path, "rb"))
    #
    print('\n ------ Spotting Feature Extraction & Pre-processing ------')
    # final_dataset_spotting = feature_extraction_spotting(dataset_name, final_images, k)
    spot_save_path = os.path.join(dataset_name + "_dataset_spotting.pkl")
    # pickle.dump(final_dataset_spotting, open(spot_save_path, "wb")) # To save time when needed to run for several attempts
    final_dataset_spotting = pickle.load(open(spot_save_path, "rb" ))
    # x = final_dataset_spotting[0][0]
    # plt.imshow(x)
    # plt.show()
    # u = final_dataset_spotting[0][0][:,:,0]
    # v = final_dataset_spotting[0][0][:,:,1]
    # gg = final_dataset_spotting[0][0][:,:,2]
    # plt.imshow(u)
    # plt.show()
    # plt.imshow(v)
    # plt.show()
